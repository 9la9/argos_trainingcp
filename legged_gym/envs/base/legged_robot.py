...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        if self.cfg.env.debug_viz == True:
            self.debug_viz = True                 
        else :
    ...
    def check_termination(self):
        foot_stuck_flag = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1., dim=1)
        self.foot_clearance_flag[:, :9] = self.foot_clearance_flag[:, 1:].clone()
        self.foot_clearance_flag[:, 9:] = foot_stuck_flag.unsqueeze(-1)
        self.reset_buf = torch.all(self.foot_clearance_flag, dim=1)
        self.time_out_buf = self.episode_length_buf > self.max_episode_length                                   
        self.reset_buf |= self.time_out_buf
        self.reset_buf |= (self.projected_gravity[:,2] > -0.3)
    ...
    def compute_observations(self):
        ...
        motor_strengths_scale, motor_strengths_shift = get_scale_shift(self.cfg.domain_rand.motor_strength)
        com_displacements_scale, com_displacements_shift = get_scale_shift(self.cfg.domain_rand.com_displacement_range)
        self.privileged_obs_buf = torch.cat(
            (
                self.heights+(2 * torch.rand_like(self.heights) - 1) * 0.2,
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_com_displacement:
            if env_id==0:
                com_displacement_range = self.cfg.domain_rand.com_displacement_range
                num_buckets = 64*5
                bucket_ids = torch.randint(0, num_buckets, (self.num_envs, 3))
                com_displacement_buckets = torch_rand_float(com_displacement_range[0], com_displacement_range[1], (num_buckets, 1), device=self.device)
                self.com_displacements = com_displacement_buckets[bucket_ids]
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        self.com_displacements[env_id, 2]+=0.03
        self.com_displacements[env_id, 0]+=0.03
        ...
        if self.cfg.domain_rand.randomize_base_inertia:
            ...
            props[0].inertia.x.x *= np.random.uniform(rng[0], rng[1])
            props[0].inertia.y.y *= np.random.uniform(rng[0], rng[1])
            props[0].inertia.z.z *= np.random.uniform(rng[0], rng[1])
        return props
    ...
    def _resample_commands(self, env_ids):
        ...
        if self.cfg.env.play_command == True:
        ...
        mask = torch.where((self.terrain_levels[env_ids] > 5), 0.6, 1.)
        self.commands[env_ids, :2] *= (torch.norm(self.commands[env_ids, :2], dim=1) > 0.2).unsqueeze(1)
        self.commands[env_ids, :3] *= mask.unsqueeze(-1)
    ...
    def _compute_torques(self, actions):
        ...
        if control_type=="P":
            torques = self.p_gains* self.Kp_factors *(self.lag_buffer[choice] + self.default_dof_pos - self.dof_pos + self.motor_offsets) -  self.d_gains* self.Kd_factors *self.dof_vel
        elif control_type=="V":
    ...
    def _init_buffers(self):
        ...
        self.forces = torch.zeros((self.num_envs, self.num_bodies, 3), device=self.device, dtype=torch.float)
        self.force_positions = self.rb_positions.clone()
        self.foot_clearance_flag = torch.zeros((self.num_envs, 10), device=self.device, dtype=torch.bool)
    ...
    def _create_envs(self):
        ...
        hip_names = [name for name in self.dof_names if "HipX_joint" in name]
        if not hip_names:
            hip_names = [name for name in self.dof_names if name.endswith("_hip_joint")]
    ...
    def _reward_ang_vel_xy(self):
        mask = torch.where((self.terrain_levels > 3), 0.5, 1.)
        return mask*torch.sum(torch.square(self.base_ang_vel[:, :2]), dim=1)

    def _reward_ang_acc_xy(self):
        return torch.sum(torch.square((self.base_ang_vel[:, :2] - self.last_base_ang_vel[:, :2]) / self.dt), dim=1)
    ...
    def _reward_orientation(self):
        ...
        self.pitch_est = self.pitch_est.clone()*0.3+0.7*pitch_est_clip
    ...
    def _reward_base_height(self):
        mask = torch.where((torch.abs(self.pitch_est) > 0.2), 0.3, 1.)
        base_height = torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights[:, 55:132], dim = 1)
        return mask*torch.square(base_height - self.cfg.rewards.base_height_target)
    ...
    def _get_foot_clearance(self, env_ids = None):
        ...
        heights_tensor = torch.stack((heights1, heights2, heights3, heights4, heights5, heights6, heights7, heights8), dim=0)
    ...
    def _reward_foot_height(self):
        height = torch.clip(self.measured_foot_clearance - 0.08, min = 0.)
        contact = self.contact_forces[:, self.feet_indices, 2] > 1.
        contact_filt = torch.logical_or(contact, self.last_contacts) 
        return torch.sum(height*~contact_filt, dim = 1)

    def _reward_foot_acc(self):
        contact = self.contact_forces[:, self.feet_indices, 2] > 1.
        contact_filt = torch.logical_or(contact, self.last_contacts) 
        mask = torch.where((self.terrain_levels > 5), 0.3, 1.)
        return torch.sum((  mask.view(-1,1)* (torch.norm( ((self.last_foot_velocities - self.foot_velocities) / self.dt) ,dim = -1) -  self.cfg.rewards.max_acc) ).clip(min=0.), dim=1)                                                  
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime = torch.sum(torch.clip((self.feet_air_time - 0.25), max = 0) * first_contact, dim=1)
        move_mask = torch.norm(self.commands[:, :2], dim=1) > 0.1
        turn_mask = (torch.norm(self.commands[:, :2], dim=1) < 0.2) & (torch.abs(self.commands[:, 2]) > 0.2)
        #rew_airTime *= torch.logical_or(move_mask, turn_mask).float()
        rew_airTime *= turn_mask.float()
    ...
    def _reward_feet_impact_vel(self):
        prev_foot_velocities = self.last_foot_velocities[:, :, 2].view(self.num_envs, -1)
    ...
    def _reward_feet_power(self):
        return torch.square(torch.bmm(self.contact_forces[:, self.feet_indices, :].reshape(-1,1,12) , self.foot_velocities.reshape(-1,12,1))).squeeze(-1).squeeze(-1)

    def _reward_hip_pos(self):
        return torch.sum(torch.square(self.dof_pos[:, self.hip_indices] - self.default_dof_pos[:, self.hip_indices]), dim=1)

    def _reward_torque_mean(self):
        torque_mean = torch.mean(torch.abs(self.torques), dim=0)
        hipx_mean = torch.mean(torque_mean[[0, 3, 6, 9]])
        hipy_mean = torch.mean(torque_mean[[1, 4, 7, 10]])
        knee_mean = torch.mean(torque_mean[[2, 5, 8, 11]])
        rew = torch.sum(torch.square(torque_mean[[0, 3, 6, 9]] - hipx_mean))+torch.sum(torch.square(torque_mean[[1, 4, 7, 10]] - hipy_mean))+torch.sum(torch.square(torque_mean[[2, 5, 8, 11]] - knee_mean))
        return rew
    ...
    def _reward_feet_collision(self):
        ...
        dis_fl_hl = torch.sum(torch.square(foot_positions[:, 0] - foot_positions[:, 2]), dim = 1)
        dis_fr_hr = torch.sum(torch.square(foot_positions[:, 1] - foot_positions[:, 3]), dim = 1)
        dis_fl_fr = torch.sum(torch.square(foot_positions[:, 0] - foot_positions[:, 1]), dim = 1)
        dis_hl_hr = torch.sum(torch.square(foot_positions[:, 2] - foot_positions[:, 3]), dim = 1)
        # print(dis_fl_hl , dis_fr_hr)
        min_feet_distance = 0.04
        min_feet_distance_sq = min_feet_distance * min_feet_distance
        return (dis_fl_hl < min_feet_distance_sq).float() + (dis_fr_hr < min_feet_distance_sq).float() + (dis_fl_fr < min_feet_distance_sq).float() + (dis_hl_hr < min_feet_distance_sq).float()
